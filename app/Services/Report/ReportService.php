...
class ReportService
{
	...
	public function getReport($responseBody)
	{
		try {
			...
			$topPurchasers = Order::with(['customer', 'product'])
				->selectRaw('sum(purchase_quantity*product_price) as total_purchase,customer_id,product_id')
			...
			$grossTotalPurchaseQuantity = Order::all()->sum('purchase_quantity');
			...
			$data = compact('topPurchasers', 'grossTotalPurchaseQuantity', 'grossTotalProductPrice', 'grossTotalPurchaseAmount');
